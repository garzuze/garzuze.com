...
const ServiceModal = ({ isOpen, onClose, project }: ProjectModal) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 bg-black/90 bg-opacity-50 z-99 transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0"
          }`}
        onClick={onClose}
      />
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div
          className={`bg-white dark:bg-stone-950 border border-stone-200 dark:border-stone-900 shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto transform transition-all duration-300 ${isOpen ? "scale-100 opacity-100" : "scale-95 opacity-0"
            }`}
        >
          {/* Modal header */}
          ...
          <div className="p-6">
            {project.fullDescription.split("\n").map((line, id) => (
              <p className="mb-6 leading-relaxed" key={id}>{line}</p>
            ))}
